...
router.get('/', (req, res) => {
  ...
  let sql = 'SELECT id,upper_code,upper_name,project_code,state,create_date,remark FROM B_Upper_Monitor'
  let sqlParams = []
  const upperCode = req.query.upperCode
  if (upperCode) {
    sql += ' WHERE upper_code=?'
    sqlParams.push(upperCode)
  }
